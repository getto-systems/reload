...
var server = http.createServer(function (req, res) {
  ...
  var optionalHeaders = []
  if (headersJson) {
    JSON.parse(fs.readFileSync(headersJson)).forEach(function (header) {
      if (header.length < 2) {
        console.log("ignored: ", header)
      } else {
        var key = header[0]
        var value = header[1]
        if (value.join) {
  ...
  var addOptionalHeaders = function (res) {
    if (optionalHeaders) {
